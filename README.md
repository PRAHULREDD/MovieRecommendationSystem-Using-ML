...
- Go to: [MovieLens Dataset on Kaggle](https://www.kaggle.com/datasets/rounakbanik/the-movies-dataset)
- Download and extract to: `data/the-movies-dataset/`
- Required files:
  - `movies_metadata.csv`
  - `credits.csv` 
  - `keywords.csv`
...
```
**This will:**
- Process 45,000+ movies
- Generate similarity matrix
- Create `backend/model_full.parquet` (1.5GB)
- Create `backend/movie_database_full.parquet` (5MB)
- Take ~10-15 minutes on modern hardware
...
```bash
# If you already have model_full.parquet and movie_database_full.parquet
pip install -r backend/requirements.txt
npm install
...
**Training Process:**
- **Input**: Raw MovieLens CSV files (230MB)
- **Processing**: Feature engineering, text processing, stemming
- **Output**: Similarity matrix (1.5GB) + Processed dataset (5MB)
- **Time**: 10-15 minutes on modern hardware
- **Memory**: Requires 4GB+ RAM during training

**Training Script Features:**
- Automated data cleaning and merging
- Feature engineering (genres, cast, director, keywords)
- Text normalization and stemming
- Cosine similarity matrix computation
- Optimized Parquet export for fast loading
...
├── backend/
│   ├── backend.py
│   ├── model_full.parquet          # Generated by model_train.py
│   ├── movie_database_full.parquet # Generated by model_train.py
│   └── requirements.txt
├── src/
│   ├── components/
│   │   ├── Loader.tsx
...
│   └── types.ts
├── data/
│   └── the-movies-dataset/         # Download from Kaggle
│       ├── credits.csv
│       ├── keywords.csv
│       ├── movies_metadata.csv
│       └── links.csv
...
4. **Missing model files:**
   - Run `python model_train.py` to generate model files
   - Ensure dataset is downloaded to `data/the-movies-dataset/`
   - Model files are generated locally (not in repo due to size)

5. **Training script errors:**
   - Ensure all CSV files are in `data/the-movies-dataset/`
   - Check you have 4GB+ RAM available
   - Install missing dependencies: `pip install nltk scikit-learn fastparquet`
